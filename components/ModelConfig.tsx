...
export const ModelConfig: React.FC<Props> = ({ selectedModel, onSelectModel, apiKey, onApiKeyChange }) => {
  return (
    <div className="space-y-6">
      {/* 1. Model Selection */}
      <div>
        <label className="block text-sm font-bold text-gray-700 mb-3 uppercase tracking-wider">1. Select AI Model</label>
        <div className="grid grid-cols-3 gap-3">
          {AI_MODELS.map((model) => (
            <button
              key={model.id}
              onClick={() => onSelectModel(model.id)}
              className={`relative p-3 rounded-2xl border-2 transition-all duration-300 flex flex-col items-center justify-center text-center gap-2 group hover:shadow-lg ${selectedModel === model.id
